...
class PublicScan:
    ...
    def Public_masscan(self):
        ...
        os.system('masscan -iL %s -p443 --rate=%s --excludefile conf/exclude.txt -oL tmp/tempResult' % (self.file, self.rate))
    ...
    def checkip(self, ip):
        ...
        p = re.compile('^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\/([0-9]|[1-2][0-9]|3[0-2]))$')
        if p.match(ip):
            return True
        else:
            return False

    def get_ip_list(self, logger):
                ...
                for line in f:
                    if line.strip():
                        if self.checkip(line.strip()):
                            ...
                            logger.infostring('This line CIDR ip maybe wrong:' + line.strip())
                            continue
            return True
        else:
            return False
    ...
    def run(self):
        ...
        if not self.get_ip_list(logger):
        ...
        logger.infostring('start parse results...')
        if self.parseResult():
            logger.infostring('program exits')
